...
class GameScene2 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('bird','assets/redbird.png');  // Mudado para redbird.png
        this.load.image('key','assets/key.png');
        this.load.image('nave','assets/nave.png');  // Mudado para nave.png
        this.load.image('door','assets/door.png');
        this.load.image('tiles','assets/tileset.png'); //carregar todos os assets como mapa, titulo
    ...
    create() {
        ...
        this.player = this.physics.add.sprite(100, 100, 'bird').setScale(0.4);  // Colocar a posicao do caranguejo e o tamanho
        this.player.setCollideWorldBounds(true);
        this.player.setBounce(0.2);
        
        this.spawnKey();
        this.door = this.physics.add.sprite(500, 200, 'door');              //Onde a chava vai aparecer no mapa, em luagres variados
        this.physics.add.overlap(this.player, this.door, this.enterDoor, null, this);
        
        this.enemy = this.physics.add.sprite(400, 200, 'nave');      //Vai colocar agora o tubarao para ter colisao com o caranguejo
        this.enemy.setVelocity(100, 100);
        this.enemy.setBounce(1, 1);
        this.enemy.setCollideWorldBounds(true);
    
        this.scoreText = this.add.text(16, 16, 'Placar: 0', { fontSize: '32px', fill: '#fff' });   //Placar do jogo
    ...
    update() {
        ...
        if (this.cursors.left.isDown) {                 //Movimentacao e velocidade
            this.player.setVelocityX(-160);
        } else if (this.cursors.right.isDown) {
            this.player.setVelocityX(160);
        }
    ...
    collectKey(player, key) {
        this.score += 10;
        this.scoreText.setText('Placar: ' + this.score);            //coletar a chave para enterar na porta
        key.destroy();
        this.hasKey = true;
    }
    
    enterDoor(player, door) {  //Porta para passar de fase so se tiver chave
        if (this.hasKey) {
            this.scene.start('GameScene2');
        }
    }
